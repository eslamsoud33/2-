...
export default function App() {
  ...
  const toggleGlobalAudio = () => {
    ...
    if (isGlobalAudioPlaying) {
      ...
      globalAudioRef.current.play().catch((err: any) => console.log('Playback error:', err));
    }
  };

  const handleGlobalAudioTimeUpdate = (e: React.SyntheticEvent<HTMLAudioElement>) => {
    // تحديث حالة الوقت فقط عند تغير الثانية الصحيحة لتجنب ثقل المتصفح (Lag) أثناء تشغيل الصوت
    const currentTime = e.currentTarget.currentTime;
    setGlobalAudioTime((prev: number) => Math.floor(prev) !== Math.floor(currentTime) ? currentTime : prev);
  };
  ...
  const handleInstallClick = async () => {
    ...
    if (!deferredPrompt) {
      showAlert("يُرجى الانتظار قليلاً حتى يكتمل تحميل ملفات التطبيق، أو فتح التطبيق في متصفح Chrome على نظام Android للتمكن من تثبيته كتطبيق حقيقي على الهاتف 📱.");
      return;
    }
    ...
    try {
      ...
      showAlert("يرجى النقر على خيارات المتصفح (النقاط الثلاث ⁝ في أعلى اليسار) واختيار 'تثبيت التطبيق' أو 'إضافة إلى الشاشة الرئيسية' 📱.");
    }
  };
      ...
      try {
        ...
        if(result.status === 'success') {
          if(result.data) {
            ...
            try {
              const savedUser: Student = JSON.parse(savedUserStr);
  ...
  useEffect(() => {
    ...
    if (savedUser && savedPhone) {
      try {
        const st: Student = JSON.parse(savedUser);
        setStudentData(st);
  ...
  const toggleUnit = (unitId: string) => setExpandedUnits((prev: string[]) => prev.includes(unitId) ? prev.filter((id: string) => id !== unitId) : [...prev, unitId]);
      ...
      if (qData.status === 'success') {
        ...
        if (session) {
          try {
            ...
            builtData.forEach((u: Unit) => { 
              const l = u.lessons.find((l: Lesson) => l.id === parsedSession.lessonId); 
              if (l) foundLesson = l; 
            });
            if (foundLesson) {
  ...
  const handleActivateMonth = async (phone: string, name: string, month: string) => {
    showConfirm(`تفعيل شهر (${month}) للطالب: ${name}؟`, async () => {
      try {
        ...
        setStudentsList((prev: Record<string, Student | Student[]>) => {
          const newList = {...prev};
          if(newList[phone]) {
            const listArr = Array.isArray(newList[phone]) ? newList[phone] : [newList[phone]];
            newList[phone] = (listArr as Student[]).map(s => {
              if (s.name !== name) return s;
              const newMissing = s.missingMonths ? s.missingMonths.filter(m => m !== month) : [];
              return { 
                ...s, 
                missingMonths: newMissing, 
                isPaid: newMissing.length === 0 
              };
            });
          }
          return newList;
  ...
  const handleApproveSubscription = async (phone: string, name: string) => {
    showConfirm(`تفعيل حساب: ${name}؟`, async () => {
      ...
      setAdminPendingReqs((prev: PendingRequest[]) => prev.filter((req: PendingRequest) => req.phone !== phone || req.name !== name));
      try { 
        await fetch(API_URL, { 
          method: 'POST', 
          mode: 'no-cors', 
  ...
  const handleTouchEnd = () => {
    ...
    if (startX === null || endX === null || isDrawingMode || !activeLesson) return;
  ...
  const handleFinishQuiz = () => {
    if (activeLesson) {
      ...
      const correct = userAnswers.filter((a: UserAnswer) => a && a.isCorrect).length;
  ...
  const renderScreen = () => {
    if (screen === 'returning_welcome') {
      ...
      let savedStats = null;
      try {
        savedStats = savedStatsStr ? JSON.parse(savedStatsStr) : null;
      } catch(e) {}
      const session = localStorage.getItem('savedSession');
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-2xl text-center max-w-md w-full border-t-8 border-secondary animate-fade-in relative overflow-hidden">
            ...
            {savedStats && savedStats.completed > 0 && (
              <div className="mb-6 relative z-10 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                ...
                لقد أنجزت <span className="text-secondary font-black text-lg mx-1">{toEasternArabic(savedStats?.completed || 0)}</span> من أصل <span className="text-primary font-black text-lg mx-1">{toEasternArabic(stats.total || savedStats?.total || 0)}</span> دروس بنجاح!
                </p>
              </div>
            )}
          ...
          <div className="w-full max-w-2xl flex flex-col items-center gap-4 sm:gap-5 py-6 z-10 my-auto">
            ...
            <div className="w-full max-w-xs sm:max-w-sm md:max-w-md bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-6 md:p-8 pt-10 border border-gray-100 flex flex-col items-center gap-4 relative mt-2">
              ...
              {showInstallBtn && (
                ...
                >
                  <i className="fas fa-mobile-alt text-lg"></i> تثبيت التطبيق على الهاتف 📱
                </button>
              )}
            ...
            <div className="flex flex-col items-end gap-1.5 shrink-0 w-full max-w-[65%] sm:max-w-[55%]">
              <div className="flex items-center gap-1.5 w-full justify-end" style={{ paddingRight: '0px', paddingLeft: '-2px', height: '23px', width: '177.797px' }}>
                {isTeacherAdmin && (
                 ...
                 {showInstallBtn && (
                  <button onClick={handleInstallClick} className="px-2 py-0.5 md:py-1 rounded-md flex items-center justify-center text-[10px] sm:text-xs md:text-sm shadow-sm transition-all shrink-0 bg-emerald-600 text-white hover:bg-emerald-700 border border-emerald-500 animate-pulse cursor-pointer font-black gap-1" title="تنزيل التطبيق على الهاتف">
                    <i className="fas fa-mobile-alt"></i> تنزيل التطبيق 📱
                  </button>
                )}
              ...
              <div className="flex justify-end items-center gap-1.5 w-full overflow-x-auto flex-nowrap custom-scrollbar pb-0.5 mt-0.5">
                ...
                <button 
                  onClick={() => showConfirm('هل أنت متأكد من مسح جميع إجاباتك والبدء من جديد؟', () => {
                    ...
                    setMistakesBank([]);
                    const resetData = data.map((unit: Unit) => ({ ...unit, lessons: unit.lessons.map((l: Lesson) => ({ ...l, completed: false })) }));
                    setData(resetData); 
                    setActiveFilter('all'); 
                  })} 
          ...
          <div className="flex-1 overflow-y-auto p-4 pt-0 z-10">
            <div className="max-w-4xl mx-auto space-y-4">
              {announcement && announcement.trim() !== "" && (
                <div className="bg-orange-50 border-r-4 border-gold p-4 rounded-2xl shadow-sm flex items-start gap-3 animate-fade-in mb-2" style={{ fontSize: '11px', lineHeight: '21px', marginBottom: '6px', marginTop: '2px', paddingLeft: '14px', paddingTop: '9px' }}>
              ...
              {stats.total === 0 ? (
                ...
                data.map((unit: Unit) => {
                  ...
                  if (activeFilter !== 'all' && !unit.lessons.some((l: Lesson) => activeFilter === 'completed' ? l.completed : !l.completed)) return null;
                  return (
                    <div key={unit.id} className={`rounded-2xl shadow-lg border overflow-hidden animate-fade-in backdrop-blur-md transition-all ${isDarkMode ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white/60 border-gray-150/50'}`}>
                      ...
                      {(expandedUnits.includes(unit.id) || activeFilter !== 'all') && (
                        <div className={`divide-y backdrop-blur-sm ${isDarkMode ? 'divide-gray-700/50' : 'divide-gray-100/50'}`} style={{ fontSize: '10px' }}>
                          {unit.lessons.filter((l: Lesson) => activeFilter === 'all' || (activeFilter === 'completed' ? l.completed : !l.completed)).map((lesson: Lesson) => (
                            <div 
                              key={lesson.id} 
                              onClick={() => { 
          ...
          <div className="flex-1 p-4 sm:p-6 z-10 flex flex-col">
            {activeMapUrl ? (
              ...
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4 animate-fade-in">
                {mapsList && mapsList.map((map: InteractiveMap) => (
        ...
        <div className={`h-[100dvh] flex flex-col watermark-container relative ${isDarkMode?'bg-gray-900':''}`} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          ...
          <div className="flex-1 overflow-y-auto p-4 pb-28 z-10 flex flex-col pt-8">
            <div className="max-w-4xl mx-auto w-full flex-1 flex flex-col mt-4">
              <div className={`rounded-3xl shadow-lg p-6 mb-8 relative border-2 mt-10 transition-all ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-[#fefee5] border-yellow-200/60'}`}>
                ...
                <h2 className={`text-xl sm:text-2xl font-bold mt-2 leading-relaxed ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>
                  <FormatQuestionText text={question.q} concept={question.concept} />
                </h2>
              </div>
                ...
                <div className="flex flex-col gap-4 mb-8">
                  {question.options.map((opt, idx) => {
                    let btnClass = isDarkMode
                      ? "bg-slate-800/80 border-2 border-slate-700/80 text-gray-100 p-4 rounded-2xl font-bold text-xl flex items-center shadow-sm cursor-pointer transition-all active:scale-95 hover:border-blue-500/80 hover:bg-slate-800"
                    ...
                    if (isAnswered && currentAnswer) {
                      if (opt === question!.correct) {
                        ...
                        btnClass = isDarkMode
                          ? "bg-gray-800/40 border-gray-800 text-gray-550 p-4 rounded-2xl font-bold text-xl flex items-center opacity-30 border-2"
              ...
              {question.type === 'essay' && (
                <div className="mb-8 w-full text-center">
                  {!isAnswered ? (
                    ...
                    <div className="bg-green-50 rounded-3xl p-6 border-r-8 border-green-500 text-right shadow-sm">
                      <h3 className="text-2xl font-black text-red-600 underline decoration-red-600 underline-offset-4 mb-4">الإجابة:</h3>
                      <div className="text-xl font-bold leading-relaxed">
                        <HighlightText text={question.answer} concept={question.concept} />
                      </div>
                    </div>
                  )}
                </div>
              ...
              {isAnswered && currentAnswer && question.type === 'mcq' && question.reason && (
                <div className={`mt-4 rounded-3xl p-6 shadow-md border-r-8 ${currentAnswer.isCorrect?'bg-green-50 border-green-500':'bg-red-50 border-red-500'}`}>
                  <h3 className="text-xl font-black text-red-600 underline decoration-red-600 underline-offset-4 mb-2">التعليل:</h3>
                  <div className="text-xl font-bold leading-relaxed">
                    <HighlightText text={question.reason} concept={question.concept} />
                  </div>
                </div>
              )}
    ...
    if (screen === 'result' && activeLesson) {
      ...
      const correct = userAnswers.filter((a: UserAnswer) => a && a.isCorrect).length; 
      const perc = Math.round((correct/totalQs)*100);
      const wrongConcepts = [...new Set(userAnswers.filter((a: UserAnswer) => a && !a.isCorrect).map((a: UserAnswer) => a.paragraph || a.concept || 'مراجعة عامة'))];
            ...
            <div className="w-full flex flex-col gap-3">
              <button 
                ...
                try {
                  ...
                  localStorage.setItem('completedLessons', JSON.stringify([activeLesson.id]));
                }
                setData(data.map((u: Unit)=>({...u,lessons:u.lessons.map((l: Lesson)=>l.id===activeLesson.id?{...l,completed:true}:l)}))); 
                  }
                  setScreen('index');
                }} 
              ...
              {perc < 100 && (
                <button 
                  onClick={()=>{
                    const wrongQs = activeLesson.questions.filter((q: Question, idx: number) => userAnswers[idx] && userAnswers[idx]?.isCorrect === false);
                    const newLesson = { ...activeLesson, questions: wrongQs };
                    setData(data.map((u: Unit)=>({...u,lessons:u.lessons.map((l: Lesson)=>l.id===activeLesson.id?newLesson:l)})));
              ...
              {adminActiveTab === 'results' && (
                <div className="bg-white rounded-2xl shadow-md overflow-hidden">
                  ...
                  <div className="p-4 space-y-2">
                    {adminResults.map((student: GroupedResult, i: number) => (
                      <div key={i} className="border border-gray-200 rounded-xl overflow-hidden">
                        ...
                        {expandedAdminStudent === student.name && (
                          <div className="divide-y divide-gray-100">
                            {student.tests.map((test: TestResult, tIdx: number) => {
                ...
                <div className="bg-white rounded-2xl shadow-md overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {adminPendingReqs.map((req: PendingRequest, i: number) => (<tr key={i}><td className="p-4 text-sm">{req.date}</td><td className="p-4 font-black">{req.name}</td><td className="p-4 font-bold" dir="ltr">{req.phone}</td><td className="p-4">{req.method}</td><td className="p-4 text-center"><div className="flex gap-2 justify-center"><button onClick={()=>handleApproveSubscription(req.phone, req.name)} className="bg-green-500 text-white px-3 py-1 rounded-lg text-sm shadow">موافقة</button><button onClick={()=>handleRejectSubscription(req.phone, req.name)} className="bg-red-500 text-white px-3 py-1 rounded-lg text-sm shadow">رفض</button></div></td></tr>))}
                          ...
                          list.push(
                            <div key={`${phone}-${i}`} className="bg-red-50 border border-red-100 p-4 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-4">
                              ...
                              <div className="flex flex-wrap gap-2 justify-center">
                                {st.missingMonths.map((m: string, mIdx: number) => (
                                  <button 
                                    key={mIdx} 
                ...
                <div className="bg-white rounded-2xl shadow-md overflow-hidden max-w-lg">
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      ...
                      <input type="text" placeholder="مثال: 60 جنيهاً" className="w-full border-2 focus:border-secondary outline-none rounded-xl px-4 py-3 text-lg font-bold" value={newFeeInput} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewFeeInput(e.target.value)} />
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <textarea 
                        ...
                        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>)=>setAnnouncementInput(e.target.value)} 
                      />
    ...
    return (
      <div className="h-[100dvh] flex flex-col items-center justify-center bg-gray-50 watermark-container">
        <i className="fas fa-exclamation-triangle text-red-500 text-5xl mb-4"></i>
        <h2 className="text-xl font-black text-gray-800 mb-4">عذراً، حدث خطأ غير متوقع</h2>
        <button onClick={() => setScreen('home')} className="bg-primary text-white font-bold py-3 px-8 rounded-xl shadow-md active:scale-95">العودة للرئيسية</button>
      </div>
    );
  };
    ...
    <React.Fragment>
      ...
      {showIOSInstallGuide && (
        <div className="fixed inset-0 bg-black/80 z-[1000] flex items-center justify-center p-4 animate-fade-in" dir="rtl">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl text-center relative">
            <button 
            ...
            <i className="fab fa-apple text-6xl text-gray-800 mb-4 block"></i>
            <h2 className="text-xl sm:text-2xl font-black text-primary mb-3">تثبيت التطبيق على آيفون / آيباد 📱</h2>
            <p className="text-gray-700 font-bold mb-6 text-sm sm:text-base leading-relaxed">
              لتثبيت هذه المنصة كتطبيق سريع ومستقل على جهاز Apple الخاص بك، اتبع الخطوات البسيطة التالية:
            </p>
      ...
      {globalAudio && (
        ...
        >
          <div className="flex items-center justify-between gap-2.5">
            <div className="flex items-center gap-2 overflow-hidden max-w-[190px]">
